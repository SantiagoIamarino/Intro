    ...
    <form id='post_form' enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>"
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" name="title"
          value="<?php echo (isset($post['title'])) ? $post['title'] : '' ?>" id="title"
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" name="slug"
          value="<?php echo (isset($post['slug'])) ? $post['slug'] : '' ?>" id="slug" placeholder="articulo-numero-uno">
      </div>

      <div class="mb-3">
        <label for="metaDescription" class="form-label">Meta description</label>
        <input type="text" class="form-control" name="metaDescription"
          value="<?php echo (isset($post['metaDescription'])) ? $post['metaDescription'] : '' ?>" id="metaDescription"
      ...
      <?php 
                    if(isset($post['content']) && !empty($post['content'])){
                        echo '<script>setTimeout(() => editor.root.innerHTML = "' . str_replace('"', "'", $post['content']) . '", 500)</script>';
                    }
      ...
      <div class="w-100 save-btn d-flex justify-content-center mt-4">
        ...
        <button class='save-btn' onclick='getContent()' type='button' name='postEdit'>
          <i class="bi bi-pencil"></i>
          Editar artículo
        </button>
        <?php else: ?>
        <button onclick='getContent()' type='button' style='min-width: 300px' class='save-btn' name='postEdit'>
